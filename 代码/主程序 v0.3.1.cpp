...
int JudgeCycle(void), MainCycle(int), MainExtract(array<int, 55>),
    GetSex(int), Requirement(void),DefineMember(GroupStruct,int); //定义判断循环，定义主要的循环,定义获得随机数的函数
const int ClassNumber = 55, GroupTotal = 8;
void output(int), GroupOutput(void), GroupCycle(void);                //定义输出姓名的函数
array<int, ClassNumber> member = {};
int main()
{
    MainCycle(JudgeCycle());  //使用主循环和默认设置
    return 0;
}

int JudgeCycle()
{
    int AutoSetting, KeepRunning = 1, repeat;
    while (KeepRunning == 1)
    {
        cout << "选择程序运行用途：" << endl << "1、重复抽签" << endl << "2、不重复抽签" << endl << "3、排座位" << endl << endl;
        cin >> AutoSetting;
        cout << endl;
        if (AutoSetting == 1 || AutoSetting == 2 || AutoSetting == 3)
        {
            repeat = MainCycle(AutoSetting);
            if (repeat == 0)
                continue;
            break;
        }
...
int Requirement(void)
{
    for (int rise1 = 0,judge1, judge2; rise1 < GroupTotal; rise1++)
    {
        ...
        if (judge1 == 1 && judge2 == 1) //两个人不在同一个组内
            return 1;
    }
    return 0;
}
...
{
    int KeepRunning = 1, RandomNumber, judge, rise1;
    while (KeepRunning == 1)  //KeepRunning的用处仅仅是为了令循环的条件能够满足
    {
        judge = 1;  //每次重新获取随机数时重新设置judge的值，防止出现死循环
        RandomNumber = rand() % ClassNumber; //随机数的范围0-54
        for (rise1 = 0; rise1 <= ClassNumber; rise1++) //遍历数组中所有的数字，从而确保一定没有重复值
        {
